...
class CollectionSetGenerator {
    static generate(agent, builder, response) {
        ...
        if (pathName.toString().matches(".*(in-octets|out-octets|high-speed|oper-status|in-pkts|out-pkts).*")){
            if (update.getVal().getValueCase().equals(Gnmi.TypedValue.ValueCase.UINT_VAL)){
                // UINT_VAL
                long value = update.getVal().getUintVal();
                log.info("INSIDE UINT_VAL -> VALUE FOR {} IS {}",pathName.toString(),value);
                ...
                log.info("INSIDE JSON_VAL -> PATH-NAME: {} for IF_LABEL {}",pathName.toString(),interfaceLabel)
                if (pathName.toString().matches(".*parent-ae-name.*")){
                    ...
                    log.info("INSIDE JSON_VAL -> VALUE FOR {} IS {} OF TYPE {} for IF_LABEL {}",pathName.toString(),jsonValLong,jsonValLong.getClass().getCanonicalName(),interfaceLabel);
